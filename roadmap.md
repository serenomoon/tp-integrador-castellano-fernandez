...
- [x] **R.1. Favicon:** El sitio web debe contar con un favicon representativo de la marca.
- [x] **R.2. Estructura de Encabezado Obligatoria:** Todas las pantallas del frontend y backend deben mostrar visiblemente:
...
- [x] **R.3. Navegación Estricta por Botones:** La navegación completa por la aplicación debe ser 100% interactiva a través de botones y enlaces visibles. **El cliente/usuario final nunca debe verse obligado a tipear una ruta a mano en la barra del navegador.**
- [x] **R.4. Diseño 100% Responsivo:** La aplicación completa debe verse y funcionar de forma premium tanto en ordenadores de escritorio como en dispositivos móviles y tablets.
- [x] **R.5. Restricción de Rubro:** Elección libre de un rubro comercial cohesivo (productos físicos o virtuales) que **NO sea de comida**.
...
- [x] **1.1. Estructura del repositorio:** Configurar la raíz del proyecto delimitando la frontera de desarrollo:
  - Carpeta `backend/` (Servidor Node.js + Express, vistas EJS y API JSON).
  - Carpeta `frontend/` (SPA o archivos estáticos del autoservicio).
- [x] **1.2. Inicialización del entorno Node.js:** 
...
- [x] **1.3. Servidor Express base:** Configurar `app.js` configurando los middlewares para parsear JSON, URL-encoded, habilitar CORS y servir la carpeta pública de archivos estáticos.
- [x] **1.4. Configuración del motor de vistas:** Establecer EJS (`app.set('view engine', 'ejs')`) y la ruta correspondiente para las plantillas HTML del panel de administración.
...
- [x] **2.1. Conexión del ORM:** [Responsable: Axel] Configurar la inicialización del ORM y definir la conexión a la base de datos usando variables de entorno (`.env`).
- [x] **2.2. Diseño e implementación de Modelos:** [Responsable: Axel]
  - [x] **Modelo Producto:** `id`, `nombre`, `descripcion`, `precio`, `imagenUrl`, `categoria` (dos categorías definidas), `activo` (booleano, por defecto `true`).
  - [x] **Modelo Venta:** `id`, `nombreCliente`, `fecha` (datetime), `precioTotal`.
  - [x] **Modelo Usuario (Administrador):** `id`, `correo` (único), `contrasena` (encriptada obligatoriamente, nunca texto plano).
  - [x] **Modelo VentaProducto:** Tabla pivote con relación **Muchos a Muchos** entre `Producto` y `Venta` que guarde la `cantidad` unitaria y el `precio` al momento de la compra.
- [x] **2.3. Desarrollo de Endpoints API JSON (Responde 100% JSON válido):** [Responsable: Saulo]
  - [x] `POST /api/admin/setup` -> Creación de usuario administrador (guarda contraseña encriptada).
  - [x] `GET /api/productos` -> Traer productos activos del sistema en formato paginado.
  - [x] `GET /api/productos/:id` -> Detalle del producto individual.
  - [x] `POST /api/ventas` -> Registrar una venta exitosa en base de datos. Debe procesar e insertar en las tablas correspondientes, calculando el precio total acumulado, guardando nombre de usuario, fecha y precio final.
  - [x] `GET /api/ventas` -> Listar todas las ventas registradas junto con sus productos asociados (Eager Loading del ORM).
- [x] **2.4. Middlewares de Validación:** [Responsable: Saulo]
  - [x] Crear middlewares para validar que los datos recibidos en las peticiones POST y PUT sean correctos (validar formato de correo, precios positivos, tipos correctos y campos obligatorios).
...
- [x] **3.2. Dashboard de Administración (`/admin/dashboard`):** [Responsable: Saulo]
...
- [x] **3.3. Acciones de Administración sobre Productos:**
  - [x] **Alta de Producto (`/admin/productos/nuevo`):** [Responsable: Axel] Formulario para cargar nombre, precio, descripción, categoría y cargar un archivo de imagen.
  ...
  - [x] **Modificación de Producto (`/admin/productos/editar/:id`):** [Responsable: Axel] Formulario precargado para editar todos los datos del producto y reemplazar la imagen (reutilizar la interfaz de alta).
  - [x] **Baja Lógica / Desactivación:** [Responsable: Saulo]
  ...
  - [x] **Reactivación:** [Responsable: Saulo] Botón para activar un producto inactivo. Debe lanzar un **Modal de Confirmación** y cambiar la propiedad `activo` a `true` en la BD.
- [x] **3.4. Reporte de Ventas en Excel:** [Responsable: Saulo]
...
- [x] **4.1. Maquetación base y soporte de Temas:** [Responsable: Saulo]
...
- [x] **4.2. Flujo Completo del Cliente (Paso a Paso del PDF):**
  - [x] **4.2.1. Pantalla de Bienvenida:** [Responsable: Saulo] El cliente entra al sitio. Se muestra un input para ingresar el nombre de usuario (solo nombre). No se permite continuar hasta completar este campo.
  - [x] **4.2.2. Catálogo de Productos:** [Responsable: Saulo] Redirección automática tras completar el paso 1. Se listan los productos activos paginados y separados por las dos categorías. Cada producto muestra: imagen, nombre, precio, descripción y controles de cantidad.
  - [x] **4.2.3. Gestión del Carrito:** [Responsable: Axel]
  ...
  - [x] **4.2.4. Confirmación de Compra:** [Responsable: Axel]
  ...
  - [x] **4.2.5. Pantalla de Ticket:** [Responsable: Axel]
  ...
  - [x] **4.2.6. Reinicio de Flujo:** [Responsable: Axel] Botón "Salir" que limpia completamente el estado de la compra (carrito, nombre) y redirige de vuelta a la Pantalla de Bienvenida para un nuevo cliente.
